...
import mlx.nn as nn
import numpy as np
from tts.models.base import check_array_shape
from tts.utils import get_class_predicate
import sys
import time
...
class Model(nn.Module):
    ...
    def generate(self, text: str, voice: str='af_heart', speed: float = 1.0, lang_code: str='af', split_pattern: str=r'\n+', verbose: bool=False):
        pipeline = KokoroPipeline(model=self, repo_id=self.REPO_ID if self.repo_id is None else self.repo_id, lang_code=lang_code)
        ...
        for segment_idx, (graphenes, phonemes, audio) in enumerate(pipeline(text, voice=voice, speed=speed, split_pattern=split_pattern)):
            ...
            ms_per_sample = 1000 / sample_rate  # This gives 0.0417 ms per sample at 24kHz
